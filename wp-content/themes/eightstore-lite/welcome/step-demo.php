...
<div class="warning-msg"><?php echo __('Please install Recommended Plugins for demo to be imported completely.<br /> And Make sure you download demo in a fresh install. <br /> Your Old Data might be deleted.','eightstore-lite');?></div>
<?php
wp_enqueue_style( 'plugin-install' );
wp_enqueue_script( 'plugin-install' );
wp_enqueue_script( 'updates' );
$eightstore_lite_req_plugins = $this->eightstore_lite_req_plugins;

foreach($eightstore_lite_req_plugins as $slug=>$plugin) :
	if($plugin['bundled'] == false) {
		?>
		<div class="action-tab warning">
			<h3><?php printf( esc_html__("Install : %s Plugin", 'eightstore-lite'), $plugin['name'] ); ?></h3>
			<p><?php echo __('Please check the plugins folder inside theme and upload the zip of plugins from plugin uploader.','eightstore-lite');?></p>
		</div>
		<?php
	} else {
		...
		switch($status['needs']) {
			...
			$label = esc_html__('Install and Activate', 'eightstore-lite');
			$link = $plugin['location'];
			break;

			case 'deactivate' :
			$btn_class = 'button';
			$label = esc_html__('Deactivate', 'eightstore-lite');
			$link = admin_url('plugins.php');
			break;

			case 'activate' :
			$btn_class = 'activate-offline button button-primary';
			$label = esc_html__('Activate', 'eightstore-lite');
			$link = $plugin['location'];
			break;
		}
		?>
